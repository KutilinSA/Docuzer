...
class CustomPinCodeTextField extends StatefulWidget {
  ...
  final String? Function(String? text)? beforeTextPaste;
...
class _PinCodeTextFieldState extends State<CustomPinCodeTextField>
  ...
  void initState() {
    ...
    if (_textEditingController!.text.isNotEmpty) {
      ...
      for (var i = 0; i < widget.length; i++) {
        replaceInputList[i] = _textEditingController!.text.length > i ? _textEditingController!.text[i] : '';
      }

      _selectedIndex = _textEditingController!.text.length;
                ...
                onLongPress: widget.enabled
                  ...
                  if (data?.text?.isNotEmpty ?? false) {
                    if (widget.beforeTextPaste != null) {
                      final processedData = widget.beforeTextPaste!(data!.text);
                      if (processedData != null) {
                        await _showPasteDialog(processedData);
                      }
                    } else {
  ...
  Color _getColorFromIndex(int index) {
    ...
    if (((_selectedIndex == index) ||
      ...
      var relevantActiveColor = _pinTheme.activeColor;
      if (isInErrorMode) relevantActiveColor = _pinTheme.errorBorderColor;
      return relevantActiveColor;
    }

    var relevantInActiveColor = _pinTheme.inactiveColor;
